...
export default function Pokemons({ pokemons, loading, error, paginator, handleNext, handlePrevious, getPokemonDetails}) {
   ...
   const PokemonsDisplay = pokemons.map((pokemon) => { 
    ...
    return(
        <a onClick={() => getPokemonDetails(pokemonId)} key={pokemonId} className="cursor-pointer flex flex-col items-center justify-center p-2 text-gray-700 max-w-xs border border-red-400 rounded-xl shadow-2xl hover:transform hover:scale-105 hover:border-2 transition-all duration-200">
